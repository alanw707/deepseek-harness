...
命令中心 Host API 是仅回环的，并与组合后的 DSH Web 应用组合。随附的 `web` 和 `web-codex` profile template 包含其 bundle，精确匹配安装所有者的 profile manifest 会升级为这些 tuple，但不会改变自定义 bundle list。原有 `/` Chat surface、session 和 client feature rows 仍是默认内容；browser face 将 Software Factory 贡献到 shell route chain 和 sidebar，Host face 只负责 API、session 和 Discord ingress。
...
可选 Discord ingress 使用专用出站 bot 连接。Direct message 接受精确配置用户发来的消息，无需服务器或频道 allowlist；guild message 要求精确的用户、服务器和频道 allowlist。Discord 可以创建、检查和取消任务，但不能批准或分派任务。Discord 任务存储其已授权回复频道和成功终止交付时间，使重启恢复能够报告中断，而不会重复通知已完成工作。
...
命令中心执行器不使用 DSH agent loop，也不发出 Session 事件，因此 recorded-Session snapshot harness 没有可表示该用户流程的 transcript。所属位置的浏览器测试覆盖 shell route 和渲染状态，真实浏览器 GIF 记录针对 loopback 服务的启动、模型执行、精确审查和应用。
...
- **要求预先配置 DM channel allowlist** — Discord direct-message channel 不是服务器 channel，且首次联系前不知道其 ID；精确用户 allowlist 已限制接受的发送者，因此单独的 DM channel list 会阻止直接使用，却不会提高发送者授权强度。

## 后果

仪表板可以提交、检查、批准、分派和取消项目任务，同时任务记录在进程重启后保留。配置的 Discord 用户可以在 direct message 中提交、检查和取消任务，allowlist Discord bot 可以在 direct message 或允许的 guild channel 中报告终止结果。代理只写入明确批准项目的私有副本；审查后应用是更改原文件夹的唯一路径。任务输出是有界且脱敏的，而非完整记录，并且精确应用有意排除二进制或超大更改。
