...
const changeReview=(task,reference)=>{const section=element('section','change-review'),heading=element('h3','', 'Review changes before applying'),intro=element('p','',reference.count+' staged change'+(reference.count===1?'':'s')+' · '+(reference.state==='applied'?'the reviewed change set is applied.':reference.state==='applying'?'the approved change set is being applied.':reference.state==='no-change'?'the executor made no changes.':reference.state==='apply-failed'||reference.state==='apply-interrupted'?'the apply outcome is recorded for recovery.':'the original project is still unchanged.')),digest=element('div','digest-row','Exact digest:'),code=element('code','',reference.sha256);digest.append(code);section.append(heading,intro,digest);if(reference.detail)section.append(element('p','change-detail',reference.detail));const review=reviews.get(task.id);if(!review&&reference.state==='pending-review'){section.append(actionButton('Review exact changes','',async button=>{button.disabled=true;reviews.set(task.id,await call('/tasks/'+encodeURIComponent(task.id)+'/changes'));await render()}));return section}if(review){review.changes.forEach((change,index)=>{const item=element('article','change-item'),title=element('h4','',change.kind.toUpperCase()+' · '+change.path),grid=element('div','diff-grid');grid.append(sideView(change.before,'Before',task.id+'-before-'+String(index)),sideView(change.after,'After',task.id+'-after-'+String(index)));item.append(title,grid);section.append(item)});if(reference.state==='pending-review'&&review.sha256===reference.sha256)section.append(actionButton('Apply these exact changes','primary change-apply',async button=>{button.disabled=true;await call('/tasks/'+encodeURIComponent(task.id)+'/apply',{method:'POST',body:JSON.stringify({sha256:review.sha256})});await render()}))}return section}
