...
const progressOf=task=>{const current=stageOf(task),finished=statusOf(task).tone==='success'&&current===3,progress=element('ol','task-progress'),labels=['New task','Review & start','Review changes','Apply'];labels.forEach((label,index)=>{const item=element('li',(finished||index<current?'done ':index===current?'current ':'')+((statusOf(task).tone==='error'&&index===current)?'current':'') ,label);progress.append(item)});return progress}
